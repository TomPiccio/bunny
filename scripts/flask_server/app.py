...
def register_routes(app, logger):
    ...
    def power_control():
        ...
        if action == 'status':
            return f"Power state: {"On" if get_power_state() else "Off"}", 200
...
def post_heartbeat():
    """Periodically report that this system is active."""
    target_url = "http://example.com/report"  # replace with your server endpoint
    while True:
        try:
            requests.post(target_url, json={"system_id": SYSTEM_UUID})
            print(f"[Heartbeat] Posted UUID: {SYSTEM_UUID}")
